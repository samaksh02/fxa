...
export class StripeWebhookHandler extends StripeHandler {
  ...
  async handlePlanUpdatedEvent(request: AuthRequest, event: Stripe.Event) {
    ...
    this.log.error('HANDLE PLAN UPDATED', { plan });
    ...
    this.log.error('HANDLE PLAN UPDATED EVENT ONLY PRODUCT', product);
    this.log.error('HANDLE PLAN UPDATED EVENT RESULT', result);
